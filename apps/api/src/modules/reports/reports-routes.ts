...
export async function reportsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/:code/run', { preHandler: authenticate }, async (request, reply) => {
    ...
    const { sql, values } = report.build(me.orgId, body.params);
    ...
    try {
      ...
      await query(
        `INSERT INTO report_executions (organization_id, report_code, params, status, format, error, created_by)
         VALUES ($1,$2,$3::jsonb,'ERROR',$4,$5,$6)`,
        [me.orgId, code, JSON.stringify(body.params), body.format, (err as Error).message, me.sub],
      );
      throw err;
    }
    ...
    await query(
      `INSERT INTO report_executions (organization_id, report_code, params, status, row_count, duration_ms, format, created_by)
       VALUES ($1,$2,$3::jsonb,'SUCCESS',$4,$5,$6,$7)`,
      [me.orgId, code, JSON.stringify(body.params), data.length, durationMs, body.format, me.sub],
    );
  ...
  app.get('/executions', { preHandler: authenticate }, async (request) => {
    ...
    const executions = await rows(
      `SELECT e.id, e.report_code, e.params, e.status, e.row_count, e.duration_ms, e.format,
              e.error, e.created_at, u.name AS created_by_name
