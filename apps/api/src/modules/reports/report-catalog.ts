 ...
 *
 * Convenção dos parâmetros posicionais no SQL:
 *   $1 → organização (conta)
 *   $2 → empresa operadora (uuid) · NULL = consolidado de todas
 *   $3 em diante → parâmetros do próprio relatório
 */
...
/** Escopo de execução: a conta e, opcionalmente, uma operadora. */
...
export interface ReportDefinition {
  ...
  build: (scope: ReportScope, params: Record<string, unknown>) => { sql: string; values: unknown[] };
}
...
/** Coluna da empresa operadora — só faz sentido na visão consolidada. */
const operatorColumn: ReportColumn = { key: 'empresa', label: 'Empresa operadora' };
...
/** Base comum: a conta e o filtro opcional de operadora. */
function base(scope: ReportScope): unknown[] {
  return [scope.organizationId, scope.operatorId ?? null];
}
...
export const REPORTS: ReportDefinition[] = [
  {
    ...
    description:
      'Relação completa dos veículos cadastrados na Track7, com identificação, grupo e situação de comunicação.',
    category: 'FROTA',
    ...
    build: (scope, params) => ({
      sql: `
        SELECT op.name                AS empresa,
          ...
          FROM vehicles v
          JOIN operators op ON op.id = v.operator_id
          LEFT JOIN vehicle_last_position p ON p.operator_id = v.operator_id AND p.asset_id = v.asset_id
          LEFT JOIN t7_groups g ON g.operator_id = v.operator_id AND g.group_id = v.site_id
         WHERE v.organization_id = $1
           AND ($2::uuid IS NULL OR v.operator_id = $2::uuid)
           AND ($3::bigint IS NULL OR v.site_id = $3::bigint)
         ORDER BY op.name, v.description NULLS LAST`,
      values: [...base(scope), params.siteId ? Number(params.siteId) : null],
    }),
  },

  {
    ...
    description:
      'Veículos que não transmitem posição há mais de N horas — indicador de equipamento com falha ou veículo recolhido.',
    category: 'OPERACAO',
    params: [{ name: 'horas', label: 'Horas sem transmitir', type: 'number', required: true, defaultValue: 24 }],
    columns: [
      operatorColumn,
      { key: 'asset_id', label: 'ID Track7' },
      { key: 'descricao', label: 'Veículo' },
      ...
      sql: `
        SELECT op.name              AS empresa,
         ...
         WHERE v.organization_id = $1
           AND ($2::uuid IS NULL OR v.operator_id = $2::uuid)
           AND v.status = 'ACTIVE'
           AND (p.recorded_at IS NULL OR p.recorded_at < now() - ($3 || ' hours')::interval)
         ORDER BY p.recorded_at ASC NULLS FIRST`,
      values: [...base(scope), Number(params.horas ?? 24)],
    }),
  },
      ...
      return {
        sql: `
          SELECT op.name              AS empresa,
            ...
            JOIN operators op ON op.id = v.operator_id
            LEFT JOIN t7_groups g ON g.operator_id = v.operator_id AND g.group_id = v.site_id
            LEFT JOIN trips t ON t.operator_id = v.operator_id
                             AND t.asset_id = v.asset_id
                             AND t.trip_start BETWEEN $3 AND $4
           WHERE v.organization_id = $1
             AND ($2::uuid IS NULL OR v.operator_id = $2::uuid)
             AND ($5::bigint IS NULL OR v.site_id = $5::bigint)
           GROUP BY op.name, v.asset_id, v.description, v.registration_number, g.name
           ORDER BY distancia_km DESC`,
        values: [...base(scope), from, to, params.siteId ? Number(params.siteId) : null],
      };
    },
  },

  {
    ...
    description:
      'Cada viagem realizada no período, com origem, destino, distância e duração — base para prestação de contas.',
      ...
      return {
        sql: `
          SELECT op.name              AS empresa,
            ...
            JOIN operators op ON op.id = t.operator_id
            LEFT JOIN drivers d ON d.operator_id = t.operator_id AND d.driver_id = t.driver_id
           WHERE t.organization_id = $1
             AND ($2::uuid IS NULL OR t.operator_id = $2::uuid)
             AND t.trip_start BETWEEN $3 AND $4
             AND ($5::bigint IS NULL OR t.asset_id = $5::bigint)
           ORDER BY t.trip_start DESC
           LIMIT 50000`,
        values: [...base(scope), from, to, params.assetId ? Number(params.assetId) : null],
      };
    },
  },
      ...
      return {
        sql: `
          SELECT op.name              AS empresa,
            ...
            JOIN vehicles v ON v.operator_id = e.operator_id AND v.asset_id = e.asset_id
            JOIN operators op ON op.id = e.operator_id
            LEFT JOIN drivers d ON d.operator_id = e.operator_id AND d.driver_id = e.driver_id
           WHERE e.organization_id = $1
             AND ($2::uuid IS NULL OR e.operator_id = $2::uuid)
             AND e.start_at BETWEEN $3 AND $4
             AND (e.event_category ILIKE '%speed%' OR e.event_description ILIKE '%veloc%'
                  OR e.event_description ILIKE '%speed%')
           ORDER BY e.start_at DESC
           LIMIT 50000`,
        values: [...base(scope), from, to],
      };
    },
  },

  {
    ...
    description:
      'Consolidado de eventos de telemetria agrupados por categoria e veículo — visão de comportamento da frota.',
    category: 'SEGURANCA',
    params: [...periodParams],
    columns: [
      operatorColumn,
    ...
    build: (scope, params) => {
      ...
      return {
        sql: `
          SELECT op.name AS empresa,
                 coalesce(nullif(e.event_category, ''), 'Não classificado') AS categoria,
                 v.description        AS veiculo,
                 v.registration_number AS placa,
...

  {
    ...
    build: (scope, params) => {
      ...
      return {
        sql: `
          SELECT op.name AS empresa,
                 coalesce(d.name, 'Não identificado') AS motorista,
...

  {
    ...
    description:
      'Dias com uso, ociosidade e aproveitamento de cada veículo no período — subsídio para dimensionamento da frota.',
    category: 'CONFORMIDADE',
    params: [...periodParams],
    columns: [
      operatorColumn,
          ...
          WITH dias AS (
            SELECT t.operator_id,
             ...
             WHERE t.organization_id = $1
               AND ($2::uuid IS NULL OR t.operator_id = $2::uuid)
               AND t.trip_start BETWEEN $3 AND $4
             GROUP BY t.operator_id, t.asset_id
          )
          SELECT op.name              AS empresa,
                 v.description        AS veiculo,
                 v.registration_number AS placa,
                 coalesce(dias.dias_com_uso, 0) AS dias_com_uso,
                 GREATEST(1, (DATE_PART('day', $4::timestamptz - $3::timestamptz) + 1))::int AS dias_periodo,
                 round(100.0 * coalesce(dias.dias_com_uso, 0)
                       / GREATEST(1, (DATE_PART('day', $4::timestamptz - $3::timestamptz) + 1))::numeric, 1)
                   AS aproveitamento_pct,
                 round(coalesce(dias.distancia_km, 0), 2) AS distancia_km
            FROM vehicles v
            JOIN operators op ON op.id = v.operator_id
            LEFT JOIN dias ON dias.operator_id = v.operator_id AND dias.asset_id = v.asset_id
           WHERE v.organization_id = $1
             AND ($2::uuid IS NULL OR v.operator_id = $2::uuid)
             AND v.status = 'ACTIVE'
